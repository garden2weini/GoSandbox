...
func getJDSkuList(skuType string, filter string) []JDSku {
	...
	tmpSql := "SELECT id,skuName,jdPrice,IFNULL(productCategory_id,0) productCategory_id,imagePath FROM JDSkus WHERE name=? AND skuName LIKE ? ORDER BY skuName"
	...
	for rows.Next() {
		...
		if err := rows.Scan(&jdSku.id, &jdSku.skuName, &jdSku.jdPrice, &jdSku.productCategory_id, &jdSku.imagePath); err != nil {
			log.Fatal(err)
			continue
		}
